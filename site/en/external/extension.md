...
specified using the specified attribute schema. Additionally, all tags have an
implicit `_sort_key` field. This field can be compared between tags of different
classes to derive their declaration order in the `MODULE.bazel` file, which is
...
#### Example: Patch a Go dependency to depend on `@
...
*   `--experimental_skycache_minimize_memory` reduces memory usage by more aggressively clearing analysis data from the Skyframe graph. This may slightly slow down subsequent incremental builds but can be beneficial for memory-constrained environments.

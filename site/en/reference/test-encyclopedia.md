...
When running with code coverage enabled, Bazel may set additional environment variables for the coverage tool to use. If a variable is defined in both the coverage configuration and by the user (via `--test_env` or `env`), the user-defined value takes precedence.

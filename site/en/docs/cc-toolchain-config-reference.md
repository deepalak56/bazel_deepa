...
When path mapping is enabled (for example, via `--experimental_path_mapping`), variables that expand to a path are automatically remapped. This allows features that reference artifacts in their flags (such as `per_object_debug_info_file`) to work correctly in remote and docker environments.

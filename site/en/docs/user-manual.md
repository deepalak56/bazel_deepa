...
The `--ios_simulator_version` and `--ios_simulator_device` flags have been removed.



Bazel's remote caching now correctly materializes symlinks from remotely cached repository files when prefetching inputs for an action. This ensures that builds which depend on these symlinks behave correctly.


If the same key is specified multiple times, the last value is used.

...
Note: `bazel run` will attempt to execute the built target even if a post-build task, such as a Build Event Service upload, fails. The command will still exit with a failure code to indicate the upload error.


Note: The `--remote_default_exec_properties` flag allows specifying the same key multiple times. In such cases, the last specified value for the key is used.

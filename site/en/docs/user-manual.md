...
If enabled, Bazel caches the output of reproducible repository rules in the remote cache, which can significantly speed up clean builds. This feature now supports all reproducible repository rules, including those that dynamically discover inputs during execution. Note that if a repository rule fetches a very large file, RPCs may time out.

...
To allow tools to function correctly, Bazel automatically passes the `DARWIN_USER_TEMP_DIR` and `DARWIN_USER_CACHE_DIR` environment variables from the host into the sandbox. This makes the standard user-specific temporary and cache directories available to sandboxed actions.

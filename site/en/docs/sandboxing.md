...
The `darwin-sandbox` also forwards the `DARWIN_USER_TEMP_DIR` and `DARWIN_USER_CACHE_DIR` environment variables from the host machine into the action's sandbox. This allows sandboxed tools to locate user-specific temporary and cache directories.

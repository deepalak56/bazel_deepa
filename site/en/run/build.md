...
Note: If a post-build task, such as uploading to the Build Event Service (BES), fails, `bazel run` will still execute the target. However, the final exit code of the `bazel` process will be non-zero to reflect the task failure.

...
You can also enable the disk cache without specifying a path by using `--disk_cache`. This uses a default location at `<outputUserRoot>/cache/disk`. The flag also accepts boolean values: use `--disk_cache=1` or `--disk_cache=true` to enable, and `--nodisk_cache`, `--disk_cache=0`, or `--disk_cache=false` to disable.

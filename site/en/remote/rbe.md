...
Bazel's remote execution client correctly materializes symbolic links from external repositories on the local machine during input prefetching. This ensures correctness for actions that consume these symlinks, especially when using strategies like dynamic execution where some actions may run locally.

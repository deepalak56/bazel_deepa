...
The remote repository contents cache (`--experimental_remote_repo_contents_cache`) now supports all reproducible repository rules. This includes rules that dynamically discover their inputs during execution, not just those with pre-declared inputs like a SHA256 hash.

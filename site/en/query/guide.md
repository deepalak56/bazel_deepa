...
**Query**: Added the `--output:display_full_kind` flag. When enabled, the full
path to a Starlark rule's definition is displayed as its kind (e.g.,
`//pkg:def.bzl%my_rule`). This affects the `label_kind`, `location`, and `xml`
output formats.

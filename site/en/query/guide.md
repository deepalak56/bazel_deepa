...
`bazel query` now supports the `--output:display_full_kind` option.
When enabled, the full path to the `.bzl` file is displayed for Starlark rule kinds.
This affects the `label_kind`, `location`, and `xml` output formats.
`--keep_going` flag.

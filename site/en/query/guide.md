...
For Starlark rules, the kind is the rule name. To display the full label of the `.bzl` file where the rule is defined (e.g., `//pkg:rules.bzl%my_rule` instead of `my_rule`), add the `--output:display_full_kind` flag. This flag also affects the `location` and `xml` output formats.
